...
import { buildPublicSubmissionStatus, getPublicSubmissionOrThrow } from "@/lib/public-api-status";
...
export async function GET(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    return Response.json({ usage: await buildTrialUsageSnapshot(client), submission: await buildPublicSubmissionStatus(submission) });
  } catch (error) {
    if (error instanceof TrialAccessError) {
      return Response.json({ error: error.message }, { status: error.status });
    }
    return Response.json({ error: error instanceof Error ? error.message : "Unexpected trial status error." }, { status: 400 });
  }
}
